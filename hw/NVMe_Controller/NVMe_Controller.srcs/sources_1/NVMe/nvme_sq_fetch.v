...
// Design Name: 
// Module Name: nvme_sq_fetch
// Project Name: SSD Controller
// Target Devices: 
// Tool Versions: 
// Description: fetch NVMe SQ data
// 
// Dependencies: 
// 
// Revision:
...
module nvme_sq_fetch #(
    ...
    output reg [511:0]           o_req_data      
);

localparam
    T_EMPTY_SLOT_BYTES = 28'h400, // 1KB, 16 entry
    SQ_ADDR_WIDTH      = 11;
...
wire         sq_bram_ren;
wire [511:0] sq_bram_rdata;

// Submission Queue data buffer in PL
pl_sq_bram pl_sq_bram (
  .clka (clk                             ),    // input wire clka
  .ena  (sq_bram_wen                     ),    // input wire ena
  .wea  (sq_bram_wen                     ),    // input wire [0 : 0] wea
  .addra(sq_bram_waddr[SQ_ADDR_WIDTH-1:5]),    // input wire [5 : 0] addra
  .dina (sq_bram_wdata                   ),    // input wire [255 : 0] dina
  .clkb (clk                             ),    // input wire clkb
  .enb  (sq_bram_ren                     ),    // input wire enb
  .addrb(sq_bram_raddr[SQ_ADDR_WIDTH-1:6]),    // input wire [4 : 0] addrb
  .doutb(sq_bram_rdata                   )     // output wire [511 : 0] doutb
);
...
assign sq_bram_ren = (rd_state == RD_IDLE) && ((sq_bram_waddr - sq_bram_raddr) >= 64);

always@(posedge clk or negedge rst_n)
if(~rst_n) begin
    sq_bram_raddr <= 'h0;
end else if(sq_bram_ren) begin
    sq_bram_raddr <= sq_bram_raddr + 64;
end
...
if(~rst_n) begin
    ...
    case(rd_state)
        RD_IDLE: begin
            if((sq_bram_waddr - sq_bram_raddr) >= 64) begin
                rd_state <= RD_GEN;
            end
        end
        RD_GEN: begin
